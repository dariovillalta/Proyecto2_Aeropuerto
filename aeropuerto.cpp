...
{
    //int arrVuelo = ui->comboBox->currentIndex();
    int numVueloCombo;
    ...
    try{
        ...
        //Vuelo vuelo, string numeroAsiento, string hora, string lugarLLegada, string lugarDestino, string clase, int numeroVuelo
        ...
        //((Vuelo)miVuelos->at(numVueloCombo)).agregarBoleto(miBoleto);
...
void Aeropuerto::cargarTabla(vector<Vuelo>* miVuelos){
    ...
    for(unsigned int i = 0; i < miVuelos->size(); i++){
        destino = QString::fromStdString( ((Vuelo)miVuelos->at(i)).getLugarDestino() );
        hora = QString::fromStdString( ((Vuelo)miVuelos->at(i)).getHora() );
        numVuelo  = ((Vuelo)miVuelos->at(i)).getNumeroVuelo();
...
void Aeropuerto::loadCombo(){
    ...
    if(miVuelos->size() > 0){
        for(unsigned int i = 0; i < miVuelos->size(); i++){
            ...
            ui->lb_avion->setText( QString::fromStdString( ((Avion)((Vuelo)miVuelos->at(i)).getAvion()).getModelo() ));
            ui->lb_destino->setText( QString::fromStdString( ((Vuelo)miVuelos->at(i)).getLugarDestino() ));
            ui->lb_hora->setText( QString::fromStdString( ((Vuelo)miVuelos->at(i)).getHora() ));
            ui->lb_numVuelo->setText( QString::number( ((Vuelo)miVuelos->at(i)).getNumeroVuelo() ) );
        }
    }else
